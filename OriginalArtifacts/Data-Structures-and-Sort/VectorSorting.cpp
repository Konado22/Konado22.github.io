...
int partition(vector<Bid>& bids, int begin, int end) {
    ...
    while (done == false) {
        ...
        while (bids[low].title.compare(pivot)) {
            ++low;
        }
        // keep decrementing high index while bids[pivot] < bids[high]
        while (pivot.compare(bids[end].title) < 0) {
            --high;
        }
...
void quickSort(vector<Bid>& bids, int begin, int end) {
    ...
    if (bids.size() <= 1) {
     ...
     midpoint is location of last element in low */
        int midpoint = partition(bids, begin, end);
...
void selectionSort(vector<Bid>& bids) {
    ...
    for (unsigned pos = 0; pos < bids.size(); pos++) {
        ...
        for (unsigned i=0; i < bids.size(); i++) {
            ...
            if (bids.at(i).title.compare(bids.at(min).title)) {
